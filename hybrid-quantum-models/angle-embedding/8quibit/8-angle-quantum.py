...
# ========================
# QUANTUM BOTTLENECK (8 QUBITS)
# ========================
class QuantumBottleneck(nn.Module):
    def __init__(self, in_channels=512, n_qubits=8, n_layers=3):
        super().__init__()
        self.n_qubits = n_qubits
        self.pool = nn.AdaptiveAvgPool2d((1,1))
        self.fc_in = nn.Linear(in_channels, n_qubits)

        dev = qml.device("default.qubit", wires=n_qubits)

        @qml.qnode(dev, interface="torch", diff_method="backprop")
        def circuit(inputs, weights):
            # PennyLane handles batched data in AngleEmbedding if inputs has shape (B, n_qubits)
            qml.AngleEmbedding(inputs, wires=range(n_qubits))
            qml.StronglyEntanglingLayers(weights, wires=range(n_qubits))
            # Safe return as a list for TorchLayer
            return [qml.expval(qml.PauliZ(i)) for i in range(n_qubits)]

        weight_shapes = {"weights": (n_layers, n_qubits, 3)}
        # Set output_dim explicitly to n_qubits
    ...
    def forward(self, x):
        ...
        # q_out will have shape (B, n_qubits)
...
class Generator(nn.Module):
    def __init__(self):
        ...
        # Quantum bottleneck with 8 qubits
        self.qb = QuantumBottleneck(512, 8, 3)
...
def train():
    ...
    train_path = os.path.abspath(os.path.join(base_dir, "..", "..", "train_split"))
    val_path   = os.path.abspath(os.path.join(base_dir, "..", "..", "valid_train"))

    save_dir   = os.path.join(base_dir, "8quantum_ckpt")
    result_dir = os.path.join(base_dir, "8quantum_results")
    ...
    for epoch in range(start_epoch, 300):
        ...
        metrics["lossG"].append(total_g/len(train_loader))
